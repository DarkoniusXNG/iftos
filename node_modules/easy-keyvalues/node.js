...
class KeyValues {
    constructor(Key, defaultValue) {
        ...
        this.ID = createID$1();
        this.baseFilePath = '';
        this.baseAbsoluteFilePath = '';
        this.SetValue(defaultValue || '');
    }
    static SetIDEnabled(enable) {
        if (enable) {
            createID$1 = () => nanoid.nanoid();
        }
        else {
            createID$1 = () => '';
        }
 ...
 */
async function AutoLoadKeyValuesBase(rootNode, rootDir) {
    const result = rootNode.FindAllKeys('#base');
    for (const kv of result) {
        const filePath = path.join(rootDir, kv.GetValue());
        const children = await LoadKeyValues(filePath, 'utf8');
        kv.LoadBase(filePath, children.GetChildren().map((v) => v.Free()));
    }
    return result;
}
/**
 ...
 * @param rootDir root dir
 */
function AutoLoadKeyValuesBaseSync(rootNode, rootDir) {
    const result = rootNode.FindAllKeys('#base');
    for (const kv of result) {
        const filePath = path.join(rootDir, kv.GetValue());
...
async function SaveKeyValuesSync(file, kv, encoding = 'utf8') {
    ...
    for (const b of bases) {
        if (b.GetBaseFilePath() && b.GetBaseAbsoluteFilePath()) {
            const root = KeyValues.CreateRoot();
            root.SetValue(b.GetChildren().map((v) => v));
            fs.writeFileSync(b.GetBaseAbsoluteFilePath(), root.Format(), encoding);
        }
    }
}
...
exports.AutoLoadKeyValuesBase = AutoLoadKeyValuesBase;
exports.AutoLoadKeyValuesBaseSync = AutoLoadKeyValuesBaseSync;
